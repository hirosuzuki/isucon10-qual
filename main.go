...
func NewMySQLConnectionEnv() *MySQLConnectionEnv {
	return &MySQLConnectionEnv{
		Host:     getEnv("MYSQL_HOST", "10.162.13.102"),
...
func searchChairs(c echo.Context) error {
	...
	dbchar, _ := mySQLConnectionData103.ConnectDB()
	dbchar.SetMaxOpenConns(10)
	defer dbchar.Close()
	...
	err = dbchar.Select(&chairs, searchQuery+searchCondition+limitOffset, params...)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	err = dbchar.Get(&res.Count, "SELECT FOUND_ROWS()")
	if err != nil {
		c.Logger().Errorf("searchChairs DB execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func postEstate(c echo.Context) error {
	...
	defer tx.Rollback()
	fmt.Fprintf(os.Stderr, "insert estate %d\n", len(records))
	vs := []string{}
	for _, row := range records {
		rm := RecordMapper{Record: row}
		id := rm.NextInt()
		...
		vs = append(vs, fmt.Sprintf(`(%d,"%s","%s","%s","%s", %f,%f,%d,%d,%d,"%s",%d)`,
			id, name, description, thumbnail, address,
			latitude, longitude, rent, doorHeight, doorWidth, features, popularity))
	...
	sql = sql + strings.Join(vs, ",")
	fmt.Fprintf(os.Stderr, "insert estate= %s\n", sql)
	_, err = tx.Exec(sql)
	if err != nil {
		c.Logger().Errorf("failed to insert estate: %v", err)
		return c.NoContent(http.StatusInternalServerError)
...
func searchEstates(c echo.Context) error {
	...
	dbestates, _ := mySQLConnectionData.ConnectDB()
	dbestates.SetMaxOpenConns(10)
	defer dbestates.Close()
	...
	estates := []Estate{}
	params = append(params, perPage, page*perPage)
	err = dbestates.Select(&estates, searchQuery+searchCondition+limitOffset, params...)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	err = dbestates.Get(&res.Count, "SELECT FOUND_ROWS()")
	if err != nil {
		c.Logger().Errorf("searchEstates DB execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT * FROM estate WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ? AND ST_Contains(ST_PolygonFromText(%s), ST_GeomFromText(%s)) ORDER BY popularity DESC, id ASC LIMIT ?`,
		coordinates.coordinatesToText(), "concat('POINT(',estate.latitude,' ',estate.longitude,')')")
